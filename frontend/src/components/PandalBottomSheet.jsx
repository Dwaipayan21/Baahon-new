const PandalBottomSheet = ({
  ...
  const crowdColors = {
    ...
    moderate: { bg: "bg-amber-50", text: "text-amber-900", dot: "bg-amber-500" },
    high: { bg: "bg-rose-50", text: "text-rose-900", dot: "bg-rose-500" },
  };

  const crowd = crowdColors[pandal.crowdType] || crowdColors.moderate;
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex flex-col min-w-0">
          ...
          <span className="text-xs text-slate-500 truncate">
            {pandal.area} • {pandal.category.charAt(0).toUpperCase() + pandal.category.slice(1)}
          </span>
        </div>
      ...
      <div className="flex items-center gap-3 bg-slate-50/80 rounded-2xl p-2.5 border border-slate-100">
        ...
        <div className="flex flex-col justify-between flex-1 min-w-0 py-0.5 gap-1.5">
          ...
          <div className="flex items-center gap-1.5">
            ...
            >
              <span className={`w-1.5 h-1.5 rounded-full ${crowd.dot} animate-pulse`} />
              <span className="truncate">{pandal.crowdLabel}</span>
            </span>
          </div>
